namespace LibraryIndividual7Jun2024
{
    public class PopulationGenerator
    {
        ...
        /// <returns></returns>
        public Population Next(Population originalPopulation, double geneticDistanceThreshold = 1.75, double geographicDistanceThreshold = 1.75)
        {
            ...
            GeographicDistanceCalculator geographicDistanceCalculator = new GeographicDistanceCalculator();
                        ...
                        for (int member2 = 0; member2 < originalPopulationSize; member2++)
                        {
                            if (member1 != member2)
                            {
                                ...
                                if ((parent1.IsMale() && parent2.IsFemale()) || (parent1.IsFemale() && parent2.IsMale()))
                                {
                                    ...
                                    {
                                        if (geographicDistanceCalculator.CalculateDistance(parent1, parent2) < geographicDistanceThreshold * random.NextDouble())
                                        {
